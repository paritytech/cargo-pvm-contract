...
10 scaffold variants live under `examples/`, generated by `scripts/regenerate-examples.sh`. They cover all CLI generation paths:

- `example-*` — `--init-type example` (fibonacci, mytoken)
- `new-blank-*` — `--init-type new` (no sol file)
- `new-from-sol-*` — `--init-type new --sol-file ...`
- `*-alloc` / `*-no-alloc` — memory model variants
...
```bash
cd examples/example-fibonacci-alloc
env -u CARGO -u RUSTUP_TOOLCHAIN cargo build --release
```

The CI `check-examples` job verifies regeneration produces no diff.
