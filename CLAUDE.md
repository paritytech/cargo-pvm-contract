...
Solidity `struct` parameters/returns (ABI `tuple`) are supported on the **macro** path: `abi_param_rust_type` walks the ABI `components` and generates a `#[derive(SolType)]` struct for each named struct (deduped by `internalType`, nested structs emitted before their parents), handling `Point`, `Point[]`, and `Point[N]` shapes. Method lookup against the `.sol` folds case and drops underscores rather than trying to *recover* one spelling from the other. Recovery cannot be made total: the scaffolder snake_cases with `convert_case`, which is acronym-aware (`tokenURI` -> `token_uri`), while the macro's `to_snake_case` breaks before every uppercase (`token_u_r_i`) and `to_camel_case` cannot restore the capitalisation (`tokenUri`) — so `tokenURI`, i.e. any ERC-721 interface, matched nothing and the build failed with "No matching Solidity function found". This bit hand-written contracts as much as scaffolded ones, which is why the rule lives in the macro rather than in the scaffolder's generated output. Normalising both sides also subsumes the digit-boundary case (`fixed3` vs `fixed_3`), so one rule replaces three accreted clauses. It is a matching predicate only — the Solidity name used for the selector and the ABI still comes from the interface, so a looser match cannot change what is emitted, only which interface function is found.

The **DSL** path rejects tuples (it can't emit the `SolType` derive it would need) with a message pointing at `--api-style macro`. That restriction is currently broader than the underlying limitation — a *static* struct would decode through the DSL template's existing `StaticEncodedLen`/`decode_at` path; only dynamic structs genuinely can't — so narrowing it later is a refinement, not a reversal.
...
- A method whose Rust name would collide with one `impl Contract` already defines (`RESERVED_METHOD_NAMES`: `new` from the template's constructor, and `host` / `env` / `with_host` injected by `#[contract]`; keep it in sync with the `impl #struct_name` blocks in `codegen/contract.rs`). This one fails loudly — two inherent methods of a name is `E0592` — but the diagnostic points into the macro expansion rather than the interface, and `host` additionally trips `E0034` because the generated dispatch's own `this.host()` call becomes ambiguous. `route`/`deploy`/`call` are *not* reserved: they are free functions in the generated module, not inherent methods.
- Two struct fields that collapse to one Rust name after snake_casing (`myField` + `my_field`), which would emit a duplicate field.
- Two functions, or two parameters of one function, that collapse to one Rust name after snake_casing. Checked on **both** the macro and DSL paths. The DSL parameter case is the reason this is a scaffold-time error rather than a build-time one: DSL parameters are emitted as `let` bindings, so a collision *shadows* instead of clashing — the project builds clean, with no warning, and the first parameter is silently unreachable.
- A Solidity name whose snake_cased form is not a valid Rust identifier. `to_case(Snake)` drops leading underscores, so the legal Solidity names `_`/`__` become the empty string and `_9` becomes `9`; the templates interpolate the result directly, emitting `pub fn (`, `pub fn 9(`, `pub 9: u64,` or `fn 9_handler`. The check tests the identifier that is actually written, which differs per path — the DSL wraps a function name as `{name}_handler`, so `_` is fine there and `_9` is not.
...
Layers 1 and 2 cover `#[method]`s **and the `#[constructor]`**, and a `.sol` constructor that declares parameters must be implemented — the mirror of the method-side "Missing implementations" check, since the builder emits that constructor entry into `.abi.json` regardless and the default `deploy()` would decode none of it. The constructor cannot go through the method lookup — syn-solidity models it as an `Item::Function` with `FunctionKind::Constructor` and `name: None`, so the by-name `find_map` never matches — so it is cross-checked separately, after the interface is resolved. It is checked only when the `.sol` actually declares a `constructor(...)`; solc rejects one inside an `interface`, so most interfaces have none and their Rust constructor stays unconstrained. Layer 3 does not apply (a constructor has no return).

The constructor is worth guarding despite that narrow trigger because its failure mode is quieter than a method's: there is no selector, so drift cannot produce a "compiles green, uncallable" entry point. Instead the builder emits the `.abi.json` constructor entry from the `.sol` while `deploy()` decodes with the Rust types, and the deploy-time size guard is a *minimum* — so a `.sol` declaring a three-field struct against a two-field Rust struct ships two disagreeing artifacts, ignores the extra word, and mis-initializes storage without reverting.
