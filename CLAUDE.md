...
Solidity `struct` parameters/returns (ABI `tuple`) are supported on the **macro** path: `abi_param_rust_type` walks the ABI `components` and generates a `#[derive(SolType)]` struct for each named struct (deduped by `internalType`, nested structs emitted before their parents), handling `Point`, `Point[]`, and `Point[N]` shapes. The scaffolder emits an explicit `#[selector(name = "<solidity name>")]` on any method whose Solidity name is not literally its generated Rust name. The macro would otherwise have to *recover* the Solidity name by camelCasing the Rust one, and that round-trip is not total: the scaffolder snake_cases with `convert_case`, which groups acronyms (`tokenURI` -> `token_uri`), while `to_camel_case` cannot restore the capitalisation (`token_uri` -> `tokenUri`). The scaffolded project then failed its own build with "No matching Solidity function found" — for `tokenURI`, i.e. any ERC-721 interface. Naming the method explicitly routes the macro through its exact-match rename path and removes the dependency on the round-trip. Over-emitting is harmless, so the rule errs toward emitting.
