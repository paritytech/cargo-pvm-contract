...
- **Custom struct as storage value:** structs that live in `Lazy<S>` / `Mapping<_, S>` must derive **both** `SolType` (for ABI / field-layout signature) **and** `SolStorage` (for `StorageEncode`/`StorageDecode` + the `StaticStorageEncode`/`StaticStorageDecode` refinement when fully static). `SolType` alone is sufficient for ABI-only types (function parameter structs, event field structs). Struct fields may be packable scalars, `String`/`Bytes`, and — as of issue #93 — `Vec<T>` for `T: StorageArrayElement` (e.g. `Vec<U256>`, `Vec<StaticStruct>`), which lays out as Solidity's `T[]` member (length inline at the field slot, body at `keccak256(slot)`). Deriving `SolStorage` on a struct with a still-unsupported field (nested `#[derive(SolType)]` structs, tuples, fixed arrays of non-`u8`, or `Vec<u8>` / `Vec<String>`) emits a `compile_error!` at expansion time — visible to `cargo check` and `trybuild`
- **`Vec<T>` as storage value (issue #93):** `Vec<T>` for `T: StorageArrayElement` implements `StorageEncode`/`StorageDecode` (solc `T[]` layout: length at the value's slot, elements at `keccak256(slot)` with the same packing/stride as `[T; N]`), so it works directly as `Lazy<Vec<T>>`, `Mapping<K, Vec<T>>`, and as a `#[derive(SolStorage)]` struct field. `Vec<u8>` stays excluded (it's `uint8[]`, distinct from `bytes` — use `Bytes`); dynamic-element `Vec<String>` is not yet supported. **Boundary vs `StorageVec<T>`:** the by-value codec reads/writes the *whole* array per access — `Lazy<Vec<T>>::set` is O(n) SSTOREs and a read-modify-append "push" is O(n) SLOAD + O(n) SSTORE — so use it for small arrays consumed whole; use `StorageVec<T>` (same on-chain layout) for per-element access and O(1) `push`/`pop`. `try_get` on an empty array returns `None` — solc conflates empty and never-written (length slot 0), unlike `String`/`Bytes` which have a written-empty sentinel
