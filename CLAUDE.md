...
One scaffolded example project lives under `examples/example-mytoken`, generated by `scripts/regenerate-examples.sh`.

It keeps four MyToken variants as separate binaries in one Cargo project:

- `example-mytoken-macro-alloc` — `pvm_contract_macros` with alloc
- `example-mytoken-macro-no-alloc` — `pvm_contract_macros` with `no_alloc`
...
The CI `check-examples` job regenerates the examples and verifies `examples/example-mytoken` builds.
