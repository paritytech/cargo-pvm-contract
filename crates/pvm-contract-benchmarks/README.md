...
This crate builds multiple contract variants (no-alloc, with-alloc, alloy) in both debug and release profiles, measures their binary sizes, and generates comparison reports. It is used in CI to detect size regressions in pull requests.
...
This will:
1. Build all contract variants (fibonacci, mytoken) in debug and release profiles
...
2. Update `contracts` list in `src/bin/build-and-measure.rs`:
   ```rust
   let contracts = vec!["fibonacci", "mytoken", "your_contract"];
   ```
...
- `fibonacci_alloy.release.polkavm`
