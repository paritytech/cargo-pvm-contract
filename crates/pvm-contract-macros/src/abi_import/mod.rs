...
pub fn expand_function(
    ...
    let selector: TokenStream = if is_constructor {
        ...
        let sig_expr = build_method_signature_expr(
            ...
            // function name as a raw identifier and `Display` keeps the `r#`,
            // so the signature would hash as `r#move(uint256)` — silently
            // diverging from solc and every ABI consumer. Mirrors
...
mod test {
    ...
    fn keyword_named_function_and_param() {
        ...
        // The selector is built at const-eval from a signature string assembled
        // by `build_method_signature_expr`, so assert on that string's literal
        // prefix rather than on precomputed bytes: `r#move(` would hash to a
        // selector no solc-generated caller would ever send.
        assert!(
            out.contains(r#""move(""#),
