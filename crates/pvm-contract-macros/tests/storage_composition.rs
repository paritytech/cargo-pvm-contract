...
fn erc20_state_new_at_assigns_contiguous_slots() {
    let host = fresh_host();
    let state = <Erc20State as StorageComponent>::new_at(StorageKey::from_slot(5), 0, host.clone());
...
fn composed_contract_layout_matches_hand_constructed() {
    let host = fresh_host();
    let mut erc20 = <Erc20State as StorageComponent>::new_at(StorageKey::from_slot(0), 0, host.clone());
    let mut metadata = <MetadataState as StorageComponent>::new_at(StorageKey::from_slot(3), 0, host.clone());
    ...
    let mut paused = <Lazy<bool> as StorageComponent>::new_at(StorageKey::from_slot(5), 31, host.clone());
...
fn nested_storage_struct_uses_offset() {
    let host = fresh_host();
    let mut outer = <OuterState as StorageComponent>::new_at(StorageKey::from_slot(10), 0, host.clone());
