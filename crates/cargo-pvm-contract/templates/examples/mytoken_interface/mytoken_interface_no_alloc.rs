...
// Interface-composition twin of the `mytoken` benchmark: identical logic and
// `.sol`, but the ERC-20 methods are folded from an `impl IErc20Simplified for MyToken`
// via `implements(...)` instead of hand-written inherent `#[method]`s. Compare
// its size against `mytoken` to see the cost of folding vs. inherent dispatch.
#[pvm_contract_sdk::contract("MyToken.sol", buffer = 256, implements(IErc20Simplified))]
mod my_token {
    use super::*;
    ...
    pub trait IErc20Simplified {
        ...
        fn mint(&mut self, to: Address, amount: U256) -> Result<(), pvm_contract_sdk::EmptyError>;
    }
