...
// Interface-composition twin of the `mytoken` benchmark (with-alloc): identical
// logic and `.sol`, but the ERC-20 methods are folded from an
// `impl IErc20Simplified for MyToken` via `implements(...)` instead of hand-written
// inherent `#[method]`s. Compare its size against `mytoken` to see the cost of
// folding vs. inherent dispatch.
#[pvm_contract_sdk::contract("MyToken.sol", allocator = "bump", implements(IErc20Simplified))]
mod my_token {
    use super::*;
    use alloc::vec;
    ...
    pub trait IErc20Simplified {
        fn total_supply(&self) -> U256;
        fn balance_of(&self, account: Address) -> U256;
        fn transfer(&mut self, to: Address, amount: U256) -> Result<(), InsufficientBalance>;
        fn mint(&mut self, to: Address, amount: U256) -> Result<(), InsufficientBalance>;
    }
    ...
    impl IErc20Simplified for MyToken {
        fn total_supply(&self) -> U256 {
            let key = total_supply_key();
