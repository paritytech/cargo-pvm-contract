...
mod my_token {
    use super::*;
    use alloc::vec;
    use pvm_contract_sdk::{Address, SolEvent as _, StorageFlags};
    ...
    impl MyToken {
        ...
        pub fn transfer(&mut self, to: Address, amount: U256) -> Result<(), TokenError> {
            ...
            let event = Transfer { from: Address(caller), to: Address(to), value: amount };
        ...
        pub fn mint(&mut self, to: Address, amount: U256) -> Result<(), TokenError> {
            ...
            let event = Transfer { from: Address([0u8; 20]), to: Address(to), value: amount };
            self.host().deposit_event(&event.topics(), &event.data());
            Ok(())
        }
